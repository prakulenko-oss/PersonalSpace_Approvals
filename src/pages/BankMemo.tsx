                 ...
                 >
                   ...
                   {selectedBank === 'raiff' && (
                     ...
                     >
                       ...
                       >
                         <Phone size={15} color={ks.sky} />
                         Служба підтримки:
                       </Caption1>
                       <Body1 style={{ display: 'block', color: ks.gray, marginBottom: 12, lineHeight: 1.55 }}>
                         Якщо у вас є запитання, будь ласка, звертайтеся в наш Інформаційний центр —
                         колл-центр Райффайзен.
                       </Body1>
                       {[
                         {
                           title: 'Гаряча лінія — контактні номери',
                           items: [
                             { tel: '0 800 500 500', note: 'по Україні' },
                             { tel: '0 800 400 504', note: 'по Україні' },
                             { tel: '(044) 490 8888', note: 'у Києві' },
                             { tel: '(068) 490 08 88', note: 'з Київстар' },
                             { tel: '+38 044 230 99 98', note: 'дзвінки із-за кордону (за тарифами оператора)' },
                             { mail: 'info@help.raiffeisen.ua', note: 'для письмових звернень' },
                           ],
                         },
                         {
                           title: 'My Raif',
                           items: [
                             { note: 'Написати в чаті застосунку' },
                             { tel: '0 800 500 500' },
                           ],
                         },
                         {
                           title: 'Raiffeisen Business Online',
                           items: [
                             { tel: '0 800 505 770', note: 'по Україні' },
                             { tel: '0 800 400 470', note: 'по Україні' },
                             { tel: '0 800 305 770', note: 'Kyivstar' },
                             { tel: '+38 (044) 230 99 98', note: 'для дзвінків з-за кордону (за тарифами оператора)' },
                             { tel: '(044) 495 41 40', note: 'у Києві та з-за кордону' },
                             { mail: 'clientbank.support@raiffeisen.ua' },
                           ],
                         },
                         {
                           title: 'Валютне врегулювання для бізнес-клієнтів',
                           items: [
                             { tel: '0 800 500 025' },
                             { tel: '0 800 305 355', note: 'Kyivstar' },
                           ],
                         },
                       ].map(group => (
                         <div
                           key={group.title}
                           style={{ padding: '10px 0', borderBottom: `1px solid ${ks.bg2}` }}
                         >
                           <Caption1
                             style={{ color: ks.navy, fontWeight: 700, display: 'block', marginBottom: 6 }}
                           >
                             {group.title}
                           </Caption1>
                           <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                             {group.items.map((it: { tel?: string; mail?: string; note?: string }, idx: number) => (
                               <div key={idx} style={{ display: 'flex', alignItems: 'baseline', flexWrap: 'wrap', gap: '2px 8px' }}>
                                 {it.tel && (
                                   <a
                                     href={`tel:${it.tel.replace(/[^+\d]/g, '')}`}
                                     style={{ color: ks.sky, fontWeight: 600, fontSize: 14, textDecoration: 'none', whiteSpace: 'nowrap' }}
                                   >
                                     {it.tel}
                                   </a>
                                 )}
                                 {it.mail && (
                                   <a
                                     href={`mailto:${it.mail}`}
                                     style={{ color: ks.sky, fontWeight: 600, fontSize: 13.5, textDecoration: 'none' }}
                                   >
                                     {it.mail}
                                   </a>
                                 )}
                                 {it.note && (
                                   <Caption1 style={{ color: ks.gray }}>
                                     {it.tel || it.mail ? `— ${it.note}` : it.note}
                                   </Caption1>
                                 )}
                               </div>
                             ))}
